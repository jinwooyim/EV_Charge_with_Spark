# EV_Charge 
<p align="center">
<img src="src/main/resources/static/image/EV_Charge.png" alt="EV Charge 로고" width="400"/>
</p>

<h2 align="center">🔌EV_Charge: 전국 전기차 충전소 위치 및 실시간 상태 확인 웹사이트</h2>
<p>EV_Charge는 전국의 전기차 충전소 정보를 지도 기반으로 제공하며</p>
<p>사용자는 가까운 충전소를 손쉽게 찾고 실시간 충전 가능 여부를 확인하여 효율적으로 충전소를 이용할 수 있습니다.</p>
...
| **실시간 충전 가능 여부 표시** | 공공데이터 API로 실시간 충전 상태 시각화 | 중 ★ |
| **다양한 검색 기능** <br> (지역 / 현재 지도 / 키워드) | Redis 캐시로 빠른 검색 + Elasticsearch로 정확도 높은 검색 | 하 ☆ |
| **게시판 및 공지사항 기능** | 커뮤니티 게시판 + 관리자 공지사항 등록/관리 | 상 ⭐ |
| **사용자 위치 기반 길찾기 기능** | KAKAO API를 활용한 사용자 현재 위치부터 목적지까지의 경로 표시 및 경유지 제공 | 상 ⭐ |
| **시간대별 충전소 예약 기능** | 해당 충전소 30분 단위 예약기능 CRUD | 상 ⭐ |
| **실시간 데이터 분석, 차트 표시** | Spark를 이용한 로지스틱 회귀, 선형회귀를 토대로 충전소 혼잡도 예측 및 시간대별 예약현황 제공 | 상 ⭐ |
...
## ERD
![EV Charge ERD](src/main/resources/static/image/EV_Charge_ERD.png)
