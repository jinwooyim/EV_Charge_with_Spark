    ...
    <script type="text/javascript">
            ...
            kakao.maps.event.addListener(marker, 'click', function () {
                ...
                fetch(apiUrl)
                    ...
                    .then(data => { // data는 [FavoriteDTO]
                        ...
                        map.setCenter(new kakao.maps.LatLng(clickedMarkerLat, clickedMarkerLng - 0.003));
                        map.setLevel(3);
		...
		$(document).ready(function () {
		    ...
		    $(document).on("click", "#reset", function (e) {
		        console.log("현 지도에서 검색 클릭. 현재 중심:", center_lat, center_lng);
		        if (map && typeof map.setLevel === 'function') {
		            map.setLevel(3);
		        } else {
		            console.error("#reset 클릭: map 객체가 유효하지 않습니다.");
		            return;
		        }
		        ...
		        window.markers = [];
		        if (window.markerInfoMap) {
		            window.markerInfoMap = {};
		        } else {
		            window.markerInfoMap = {};
		        }
		        ...
		        .then(chargerDataList => {
		            if (!Array.isArray(chargerDataList)) {
		                 console.error("현 지도 검색 - /search_data API가 배열을 반환하지 않았습니다:", chargerDataList);
		                 alert("충전소 정보를 가져오는 데 실패했습니다. (응답 형식 오류)");
		                 return;
		            }
		            console.log("현 지도 검색 - /search_data API 응답 (EvChargerDTO list):", chargerDataList);

		            if (chargerDataList.length === 0) {
		                console.warn("현 지도 검색 - /search_data API 응답이 비어있습니다 (주변에 충전소 없음).");
		                alert("주변에 검색된 충전소가 없습니다.");
		                return;
		            }
		            ...
		            chargerDataList.forEach((charger, index) => {
		                if (charger.lat != null && charger.lng != null && !isNaN(parseFloat(charger.lat)) && !isNaN(parseFloat(charger.lng))) {
		                    ...
		                    window.markerInfoMap[key].push(charger);
		                     ...
		                     console.warn(`[현 지도 검색] 유효하지 않은 좌표를 가진 충전기 데이터 제외 (${index}):`, charger);
		                }
		            });

		            const entries = Object.entries(window.markerInfoMap);
		            console.log("[현 지도 검색] 지도에 표시할 마커 그룹 수:", entries.length);

		            if (entries.length === 0 && chargerDataList.length > 0) {
		                 console.warn("[현 지도 검색] 충전소 데이터는 있으나, 유효한 좌표를 가진 데이터가 없어 마커를 생성할 수 없습니다.");
		            }

		            entries.forEach(([key, chargersAtLocation]) => {
		                const parts = key.split(',');
		                const lat = parseFloat(parts[0]);
		                const lng = parseFloat(parts[1]);
		                if (!isNaN(lat) && !isNaN(lng) && window.addMarker_two) {
		                    ...
		                    if(isNaN(lat) || isNaN(lng)) console.warn("[현 지도 검색] 마커 생성 위한 좌표 파싱 실패:", key);
		                    if(!window.addMarker_two) console.error("[현 지도 검색] window.addMarker_two 함수 없음");
		                }
		            });
		        })
		        .catch(error => {
		            console.error("현 지도에서 검색 중 오류 발생:", error);
		            alert(error.message || "현 지도에서 충전소 검색 중 오류가 발생했습니다.");
		        });
		    });
